...
class Database {
     ...
     */
    saveProjectImage(projectImage, elfLocation) {
        ...
        return pool.connect().then(function (client) {
            return client.query('BEGIN').then(function () {
                return client.query('SELECT id FROM elf WHERE elf = $1', [elfLocation]).then(function (res) {
                    ...
                    .then(function (elfId) {
                    const componentPromises = projectImage.SWC.map(function (component) {
                        return client.query('INSERT INTO softwarecomponent (elf, name, size, date) VALUES ($1, $2, $3, $4)', [elfId, component.$.name, component.$.size, date]);
                    });
     ...
     */
    getComponentData(componentName, elfLocation) {
        return this.query(`
